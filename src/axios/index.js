...
instance.interceptors.response.use(
  // 请求成功
  res => res.status.toString().slice(0, 2) === '20' || res.status === 304 ? Promise.resolve(res) : Promise.reject(res),
  // 请求失败
  error => {
    const {
      response
...
export function get(urlAndParams) {
  return new Promise((resolve, reject) => {
    instance.get(urlAndParams).then(res => {
      resolve(res)
    }).catch(err => {
      reject(err)
    })
...
export function deletee(url, params) {
  return new Promise((resolve, reject) => {
    ...
    instance.delete(url + '?' + paramStr).then(res => {
      resolve(res)
    }).catch(err => {
      reject(err)
    })
